...
assert PySquashfsImage is not None
...
class SquashFSFile(io.RawIOBase):
    ...
    def _fullSeek(self, offset: int):
        self._offset = max(0, min(offset, self._size))
        self._refillBuffer(self._offset // self._blockSize)
    ...
    def seek(self, offset: int, whence: int = io.SEEK_SET) -> int:
        ...
        bufferSize = 0 if self._buffer is None else len(self._buffer)
        if offset >= bufferOffset and offset < bufferOffset + bufferSize:
            self._bufferIO.seek(offset - bufferOffset)
        else:
            self._fullSeek(offset)
        return self.tell()

    @overrides(io.RawIOBase)
    def tell(self) -> int:
        result = self._blockIndex * self._blockSize
        if self._bufferIO:
            result += self._bufferIO.tell()
        return result


class SquashFSImage(PySquashfsImage.SquashFsImage):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)  # Calls overriden _initialize
